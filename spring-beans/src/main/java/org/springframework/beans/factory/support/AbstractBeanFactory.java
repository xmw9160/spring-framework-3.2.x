...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T widoGetBean(
		...
		if (sharedInstance != null && args == null) {
            ...
            // 只有单例情况才会尝试解决循环依赖,原型模式情况下, 如果存在A中有B的属性, B中有A的属性,
            // 那么当依赖注入的时候,就会产生当A还未创建完成的时候因为对于B的创建再次返回创建A, 造成循环依赖,
            // 也就是下面的情况
			...
			try {
                ...
                // 如果只定BeanName是子Bean的话同时会合并父类的相关属性
                ...
                // singleton模式的创建
                // 单例在spring的同一个容器内只会创建一次,后续再获取bean,就直接从缓存中获取了
				if (mbd.isSingleton()) {
				    //XXX 原始状态的bean
                ...
                // prototype 模式创建(new)
				    ...
				    // 指定的scope上实例化bean {request、session、global session...}
		...
		if (requiredType != null && bean != null && !requiredType.isAssignableFrom(bean.getClass())) {
			try {
			    // 将类型转换为实际需要的类型 ex: String -> Integer
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		// requiresDestruction() 配置了destory-method, 需要注册以便于在销毁的时候调用
